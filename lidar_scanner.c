...
)
{
  ...
  int ret;
  int size_to_read;
  const int request_size = 29;
  char* pBuffer;
  //uint32_t start_step = 0UL;
  //uint32_t stop_step  = 100UL;
  //uint32_t cluster_count = 0UL;

  //uint8_t Commmand[2];

  //ticks = rtems_clock_get_ticks_per_second();

  while(tcpip_init_done == false) {
  ...
  /* bind to port 10940 at any interface */
  address.sin_family      = AF_INET;
  address.sin_port        = htons(10940);
  ...
  //
  for ( ; ; ) {
    //snprintf(buffer, sizeof(buffer), "GD%4lu%4lu%2\n", start_step, stop_step, cluster_count);
    //lwip_write(sock, buffer, strlen(buffer));

    size_to_read = request_size;
    pBuffer = (char*) buffer;

    while(size_to_read > 0){
      ret = lwip_read(sock, pBuffer, size_to_read);
